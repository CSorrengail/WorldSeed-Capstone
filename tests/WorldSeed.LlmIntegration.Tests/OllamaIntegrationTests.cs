...
public class OllamaIntegrationTests
{
    ...
    public async Task Sends_a_native_local_ollama_chat_request_and_reads_response()
    {
        ...
        var response = await client.CompleteAsync(new LlmChatRequest([new(LlmMessageRole.System, "Use JSON."), new(LlmMessageRole.User, "An idea")], Temperature: 0.1, MaxOutputTokens: 400, ResponseSchema: new JsonObject { ["type"] = "object" }));
